...
from (
    select to_char(created_at, 'mm') as MONTH, name, count(place_id) as COUNT, rank() over(partition by to_char(created_at, 'mm') order by count(place_id) desc) as RANK
    from places p inner join place_reviews pr on p.id = pr.place_id
    where pr.created_at between to_date('2018-01-01', 'yyyy-mm-dd') and to_date('2018-12-31', 'yyyy-mm-dd')
    group by to_char(created_at, 'mm'), name
) 
where rank = 1
order by 1, 2
